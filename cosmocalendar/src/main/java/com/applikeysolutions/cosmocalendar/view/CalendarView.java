...
import com.applikeysolutions.cosmocalendar.selection.OnDayHighLightListener;
...
public class CalendarView extends RelativeLayout implements OnDaySelectedListener, OnDayHighLightListener,
        AppearanceInterface, DateInterface, CalendarListsInterface,
        SelectionInterface, MultipleSelectionBarAdapter.ListItemClickListener,
        GravitySnapHelper.SnapListener {

    private List<Day> selectedDays;
    private List<Day> highlightDays;
...
//        ivNext.setImageResource(settingsManager.getNextMonthIconRes());
    ...
    /**
     * Returns all highlight days
     *
     * @return
     */
    public List<Day> getHighlightDays() {
        List<Day> highlight = new ArrayList<>();
        for (Iterator<Month> monthIterator = monthAdapter.getData().iterator(); monthIterator.hasNext(); ) {
            Month month = monthIterator.next();
            for (Iterator<Day> dayIterator = month.getDaysWithoutTitlesAndOnlyCurrent().iterator(); dayIterator.hasNext(); ) {
                Day day = dayIterator.next();
